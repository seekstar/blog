...
一款文献管理器。可以选择把文献导出为文件，也可以直接把文献拖到文本编辑器，或者`Ctrl+Shift+C`复制到剪切板。格式在`编辑 -> 设置 -> 导出 -> 条目格式`设置。

默认导出的bibtex似乎会带花括号做大小写保护。如果想要在导出的时候不包含花括号，可以安装`Better BibTeX`插件，在`编辑 -> 设置 -> Better BibTeX`里把`使用大括号括起首字母大写的单词以保持大小写格式`取消掉，然后在`编辑 -> 设置 -> 导出 -> 条目格式`里选择`Better BibTeX`即可。

Better BibTeX默认似乎不会导出URL。但是有些参考文献是网页，所以建议在`编辑 -> 设置 -> Better BibTeX`里把`将URL添加到BibTeX导出的`设置为`note字段`。
...
云盘客户端。但是截至`9.0.20`，flatpak版本没有单点登录。
